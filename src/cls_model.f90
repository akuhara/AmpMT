module cls_model
  ...
  type model
     ...
     integer, allocatable :: prior_type(:) ! 0: Gaussian, 1: uniform
...
contains
  ...
  subroutine model_perturb(self, i, log_prior_ratio, prior_ok)
    ...
    if (self%prior_type(i) == 0) then
       ...
       if (x_new < self%mu(i) .or. x_new > self%sigma(i)) then
          prior_ok = .false.
       end if
       log_prior_ratio = 0.0
    end if
